    ...
    useEffect(() => {
        ...
        if (token) {
            api.get('/users/me/')
                .then(response => {
                    setIsAuthenticated(true);
                    setUser(response.data); // Assuming /users/me/ returns user data
                    console.log('AuthContext useEffect: /users/me/ successful', { isAuthenticated: true, user: response.data });
                })
                .catch((error) => {
                    // Token is invalid or expired or other backend issue
                    console.error('AuthContext useEffect: /users/me/ failed', error);
                    localStorage.removeItem('access_token');
                    setIsAuthenticated(false);
                    setUser(null);
                })
